...
9. `npm run tauri build` → install the `.msi` on a second machine with no dev tools; confirm speech capture works with zero setup. **Half done: the bundle now exists.** `npm run tauri build` had never been run in this repo; it now produces `Debate Coach_0.1.0_x64-setup.exe` (4.7 MB) and `Debate Coach_0.1.0_x64_en-US.msi` (6.3 MB) in `src-tauri/target/release/bundle/`. Tauri downloads WiX and NSIS on first use, which is the one thing about a first build that needs network.

    **What the bundle contains was checked rather than assumed.** `VITE_SUPABASE_URL` and the anon key are inlined, so a teammate's install is pre-pointed at the project with no `.env` — which is the property that makes the installer worth sending. No `sk-ant` value appears anywhere in it; the string `ANTHROPIC_API_KEY` does, and is the variable *name* in the coach panel's own instructions. Whisper is not bundled, which is why it is 4.7 MB rather than 650 — that needs `--config tauri.bundle-whisper.conf.json`.

    **Still open**: the second machine, and the clause after it. Nothing has installed this, so nothing has confirmed that speech capture works with zero setup — and it *cannot* from this bundle, because a build without whisper falls back to the browser recogniser by design. Proving that clause needs the whisper-bundled build. What this installer is good for is phase 11: it is the thing a co-prep partner runs.
