...
6. **Whisper sidecar check** — **half done.** `scripts/fetch-whisper.ps1` has now run, and whisper.cpp v1.7.6 transcribes on this machine with the exact flags `transcribe_wav` passes. What that settled, measured on `samples/jfk.wav` at 4 threads:

   | | throughput | 7-minute speech |
   |---|---|---|
   | `base.en` | ~6.5× real time, ~0.15 s per audio-second | ~64 s |
   | `small.en` | ~2.2× real time, ~0.46 s per audio-second | ~3.2 min |

   - **The two transcripts differ, and `small.en` is better.** On the same audio `base.en` gives "Americans asked not what your country can do for you." and `small.en` gives "Americans, ask not what your country can do for you," — a wrong word and wrong punctuation against a right one. The re-pass earns its place, which until now was an assumption.
   - **`small.en` at 3.2 minutes for one speech confirms the re-pass cannot block anything**, which is what phase 6's `(async)` fix was for, and why the live report is shown first rather than waited on.
   ...
   **Still open**: no microphone. Everything above is file-in, file-out. What only live capture can settle is whether the window actually slides the way `fold_window` assumes on continuous speech, and whether the teleprompter's real lag is tolerable at speaking pace.
